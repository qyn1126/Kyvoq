...
- 面板未固定时，按 `Esc`、点击其他位置或切换程序会自动隐藏；固定后仅 `Esc`、关闭按钮或呼出快捷键会隐藏主面板。
- 关闭主窗口后驻留通知区域，项目快捷键仍然有效。
- 基于 WPF UI 的 Fluent 主题，支持跟随系统、浅色、深色以及系统或自定义强调色。
...
5. 右上角图钉可固定主面板；搜索面板始终在失去焦点时自动隐藏。
...
- `src/Kyvoq.App`：WPF 界面、Mica 主题、托盘、图标缓存、单实例和 Win32 全局快捷键。
